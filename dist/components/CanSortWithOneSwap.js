...
const CanSortWithOneSwap = (array) => {
    ...
    let mismatchIndices = []; // یک آرایه برای ذخیره آدرس خانه های اشتباه
    for (let i = 0; i < n; i++) {
        // مقایسه تک به تک عضو های هر دو آرایه باهم
        if (sortArray[i] !== array[i])
            mismatchIndices.push(i); // O(n)
    }
    if (mismatchIndices.length == 0)
        return true; // در صورتی که آرایه از قبل مرتب بود
    if (mismatchIndices.length !== 2)
        return false; // در صورتی که بیشتر از دو خانه اشتباه وجود داشت
...
// => O(n.log n)
exports.default = CanSortWithOneSwap;
